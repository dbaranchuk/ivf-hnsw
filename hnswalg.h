...
namespace hnswlib {
    ...
    public:
                        ...
                        size_t maxClusters = 0, size_t M_cluster = 0) : elementLevels(maxElements + maxClusters)
        {
            ...
            size_t predicted_size_per_element = size_data_per_element_ + sizeof(void *) + 8 + 2 * 8;
            size_t predicted_size_per_cluster = size_data_per_cluster_ + sizeof(void *) + 8 + 2 * 8;
...
//                for (int i = 0; i < k; i++) {
                ...
                _mm_prefetch((char *) (massVisited + *data), _MM_HINT_T1);
                _mm_prefetch((char *) (massVisited + *data + 64), _MM_HINT_T1);
                _mm_prefetch(getDataByInternalId(*data), _MM_HINT_T0);

                for (linklistsizeint j = 0; j < size; j++) {
                    int tnum = *(data + j);

                    _mm_prefetch((char *) (massVisited + *(data + j + 1)), _MM_HINT_T1);
                    _mm_prefetch(getDataByInternalId(*(data + j + 1)), _MM_HINT_T0);
