...
namespace hnswlib {
    ...
    public:
        ...
        searchBaseLayerST(tableint ep, void *datapoint, size_t ef)
        {
            ...
            dist_t dist = space->fstdistfunc((float *)datapoint, getDataByInternalId(ep),true);
            dist_calc++;
            topResults.emplace(dist, ep);
            candidateSet.emplace(-dist, ep);
            massVisited[ep] = currentV;
                ...
                for (int i = 0; i < k; i++) {
                    ...
                    for (linklistsizeint j = 1; j <= size; j++) {
                        ...
                        if (!(massVisited[tnum] == currentV)) {
                            ...
                            massVisited[tnum] = currentV;
                            dist_t dist = space->fstdistfunc((float *)datapoint, getDataByInternalId(tnum), true);
                            dist_calc++;
        ...
        {
            tableint currObj = enterpoint_node;
            dist_t curdist = space->fstdistfunc((float *)query_data, getDataByInternalId(enterpoint_node), true);
            dist_calc++;
            for (int level = maxlevel_; level > 0; level--) {
                ...
                while (changed) {
                    ...
                    for (linklistsizeint i = 0; i < size; i++) {
                        ...
                        if (cand < 0 || cand > (maxelements_ + maxclusters_))
                            throw runtime_error("cand error");
                        dist_t d = space->fstdistfunc((float *) query_data, getDataByInternalId(cand), true);
                        dist_calc++;
                        if (d < curdist) {
                            curdist = d;
                            currObj = cand;
