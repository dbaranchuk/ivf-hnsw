...
int main(int argc, char **argv)
{
    ...
    // Initialize Index 
    //==================
    IndexIVF_HNSW *index = new IndexIVF_HNSW(opt.d, opt.nc, opt.code_size, 8, opt.do_opq);
    ...
    if (exists(opt.path_pq) && exists(opt.path_norm_pq)) {
        ...
        if (opt.do_opq){
            ...
            index->opq_matrix = dynamic_cast<const faiss::LinearTransform *>(faiss::read_VectorTransform(opt.path_opq_matrix));
